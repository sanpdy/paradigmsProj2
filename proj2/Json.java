...
public abstract class Json {
    ...
    public Json getIfExists(String name) {
        return this.asObject().fieldIfExists(name);
    }
    ...
    public static class StringParser {
        ...
        void expect(String s) {
            if (!str.substring(pos, Math.min(str.length(), pos + s.length())).equals(s))
                throw new RuntimeException("Expected \"" + s + "\", Got \""
                        + str.substring(pos, Math.min(str.length(), pos + s.length())) + "\"");
            pos += s.length();
        }
                ...
                if (c == '\"') {
                    ...
                    switch (c) {
                        ...
                        case 't':
                            ...
                            break;
                        case 'u':
                            throw new RuntimeException("Sorry, unicode characters are not yet supported");
                        default:
